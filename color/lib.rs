  ...
  P: ColorParser<'i>,
{
  ...
  match *token {
    ...
    Token::Function(ref name) => {
      let name = name.clone();
      return input
        .parse_nested_block(|arguments| parse_color_function(color_parser, name, arguments));
    }
    _ => Err(()),
  }
...
pub trait FromParsedColor {
  ...
  fn from_oklab(lightness: Option<f32>, a: Option<f32>, b: Option<f32>, alpha: Option<f32>)
    -> Self;
