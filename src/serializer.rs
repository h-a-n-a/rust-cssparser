...
impl ToCss for Token<'_> {
    ...
    W: fmt::Write,
  {
    match *self {
      ...
      // Less-specific
      Token::VariableCurly(ref variable) => {
        dest.write_str("@{")?;
...
impl TokenSerializationType {
  ...
  pub fn needs_separator_when_before(self, other: TokenSerializationType) -> bool {
    ...
    match self {
      ...
      Nothing | WhiteSpace | Percentage | UrlOrBadUrl | Function | CDC | OpenParen | DashMatch
      | SubstringMatch | DelimQuestion | DelimEquals | DelimPercent | Other => false,
    }
  }
}
